...
export const initWalletProvider = async (
  ...
  /* URL BLockfrost */
  const urlBloclfrost =
    chain === DEFAULT_SUPPORT_CHAINS.CARDANO
      ? DEFAULT_BLOCKFROST_MAINNET_PROVIDER_URL
      : DEFAULT_BLOCKFROST_TESTNET_PROVIDER_URL;
  ...
  /* Network */
  const network =
    chain === DEFAULT_SUPPORT_CHAINS.CARDANO ? "Mainnet" : "Preprod";
  ...
  const lucid = await Lucid(
    new Blockfrost(urlBloclfrost, blockfrostId),
    network
  );
  /* Select wallet */
